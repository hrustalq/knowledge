...
function editClauses(
  ...
  if (changed.includes('isDraft')) {
    if (meta.isDraft === false) clauses.push(t('mr.note.markedReady'))
    else if (meta.isDraft === true) clauses.push(t('mr.note.markedDraft'))
    else clauses.push(t('mr.note.changedDraftStatus'))
  }
  if (changed.includes('assigneeId')) {
    const assignee = str(meta.assigneeId)
    if (assignee) clauses.push(t('mr.note.assignedTo', { name: nameOf(assignee) }))
    else if ('assigneeId' in meta) clauses.push(t('mr.note.removedAssignee'))
    else clauses.push(t('mr.note.changedAssignee'))
  }
  if (changed.includes('title')) clauses.push(t('mr.note.changedTitle'))
  if (changed.includes('description')) clauses.push(t('mr.note.editedDescription'))
  return clauses
}
...
export function systemNote(
  ...
  switch (entry.action) {
    case 'merge-request.created':
      return { icon: GitPullRequestArrow, tone: 'text-emerald-500', text: t('mr.opened') }

    case 'merge-request.updated': {
      const clauses = editClauses(meta, nameOf, t)
    ...
    case 'merge-request.review-requested': {
      ...
      const who = ids.length > 0 ? ids.map(nameOf).join(', ') : t('mr.note.aReviewer')
      return { icon: UserPlus, tone: 'text-sky-500', text: t('mr.note.requestedReview', { who }) }
    }

    case 'merge-request.approved':
      return { icon: CheckCircle2, tone: 'text-emerald-500', text: t('mr.approvedAction') }

    case 'merge-request.closed':
      return { icon: GitPullRequestClosed, tone: 'text-red-500', text: t('mr.closedAction') }

    case 'merge-request.reopened':
      return { icon: RotateCcw, tone: 'text-emerald-500', text: t('mr.reopenedAction') }
    ...
    case 'merge-request.merged': {
      ...
      return {
        ...
        text: strategy === 'squash' ? t('mr.note.mergedSquash') : t('mr.note.mergedCommit'),
